...
from scvi.train import AdversarialTrainingPlan, MatchingTrainingPlan
...
class MATCHVI(VAEMixin, UnsupervisedTrainingMixin, BaseModelClass, ArchesMixin):
    """Integration of 2 unparied single-modality data.

    MatchVI is used to integrate multiomic datasets with single-modality (expression
    or accessibility) datasets.
    ...
    * The main batch annotation, specified in ``setup_anndata``, should correspond to
    ...
    _module_cls = MULTIVAE
    _training_plan_cls = MatchingTrainingPlan
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        self.n_labels = len(adata.obs._scvi_labels.unique())
        if "n_proteins" in self.summary_stats:
            n_proteins = self.summary_stats.n_proteins
        self.module = self._module_cls(
            n_input_genes=n_genes,
        ...
        setattr(self.module, "n_labels", len(adata.obs["_scvi_labels"].unique()))
        self._model_summary_string = (
            "MultiVI Model with INPUTS: n_genes:{}, n_regions:{}, n_proteins:{}\n"
            "n_hidden: {}, n_latent: {}, n_layers_encoder: {}, "
            "n_layers_decoder: {} , dropout_rate: {}, latent_distribution: {}, deep injection: {}, "
    ...
    def train(
        ...
        datasplitter_kwargs: dict | None = None,
        plan_kwargs: dict | None = None,
        scale_match_loss: Optional[float] = 1.0,
        scale_adversarial_loss: Optional[float] = 1.0,
    ...
    def get_region_factors(self) -> np.ndarray:
        ...
        if self.n_regions == 0:
            ...
            if self.module.region_factors is None:
                raise RuntimeError("region factors were not included in this model")
            if self.module.accessibility_likelihood == "normal":
                return torch.exp(self.module.region_factors).cpy().numpy()
            return torch.sigmoid(self.module.region_factors).cpu().numpy()
    ...
    def get_accessibility_estimates(
        ...
        for tensors in post:
            ...
            if normalize_regions:
                if self.module.accessibility_likelihood == "normal":
                    p *= torch.exp(self.module.region_factors).cpu()
                else:
                    p *= torch.sigmoid(self.module.region_factors).cpu()
            if threshold:
                p[p < threshold] = 0
