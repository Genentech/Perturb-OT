...
class MatchingTrainingPlan(TrainingPlan):
    """Train vaes with adversarial loss option to encourage latent space mixing.
    ...
    **loss_kwargs
    ...
    def __init__(
        ...
        *,
        ...
        adversarial_classifier: Union[bool, Classifier] = False,
        scale_adversarial_loss: Union[float, Literal["auto"]] = "auto",
        match: Union[bool, Classifier] = True,
        match_metric: Literal["mmd", "cross_entropy"] = "cross_entropy",
        scale_match_loss: Union[float, Literal["auto"]] = "auto",
        transport_plan: Optional[Iterable] = None,
        **loss_kwargs,
    ):
        super().__init__(
            module=module,
        ...
        # if match_metric == "cross_entropy" and transport_plan is None:
        #     raise ValueError("transport not provided for cross entropy loss")

        self.scale_match_loss = scale_match_loss
        self.automatic_optimization = False
        # self.transport_plan = transport_plan

        assert adversarial_classifier is True
        if adversarial_classifier is True:
            self.n_output_classifier = self.module.n_batch
            self.adversarial_classifier = Classifier(
                n_input=self.module.n_latent + self.module.n_labels,
                n_hidden=32,
                n_labels=self.n_output_classifier,
                n_layers=3,
                logits=True,
    ...
    def loss_adversarial_classifier(
        ...
        """Loss for adversarial classifier."""
        n_classes = self.n_output_classifier
        onehot_labels = one_hot(label_index, self.module.n_labels)
    ...
    def loss_MMD(self, z, obs_index, batch_index, label_index, kernel="rbf"):
        """Loss for adversarial classifier.
        obs_index: index of sample to be used to fetch the relevant part in self.transprot_plan
        batch_index: indicates if the sample z is from P or Q.
        label_index: indicates the label of z for the MMD to be calculated.
        """
        mmd = torch.tensor(0.0).to(z.device)
        for l in torch.unique(label_index):
            _z_s = z[(batch_index[:, 0] == 0) & (label_index[:, 0] == l), :]
            _z_t = z[(batch_index[:, 0] == 1) & (label_index[:, 0] == l), :]
            if _z_s.shape[0] == 0 or _z_t.shape[0] == 0:
                continue
            _idx_s = obs_index[(batch_index[:, 0] == 0) & (label_index[:, 0] == l), :]
            _idx_t = obs_index[(batch_index[:, 0] == 1) & (label_index[:, 0] == l), :]

            # code adopted from https://www.onurtunali.com/ml/2019/03/08/maximum-mean-discrepancy-in-machine-learning.html
            xx, yy, xy = (
                torch.mm(_z_s, _z_s.t()),
                torch.mm(_z_t, _z_t.t()),
                torch.mm(_z_s, _z_t.t()),
            )
            ...
            dxy = (
                xx.diag().unsqueeze(1).expand_as(xy)
                + yy.diag().unsqueeze(0).expand_as(xy)
                - 2.0 * xy
            )  # Used for C in (1)

            XX, YY, XY = (
                torch.zeros(xx.shape).to(xx.device),
                torch.zeros(yy.shape).to(yy.device),
                torch.zeros(xy.shape).to(xy.device),
            )
            ...
            if self.transport_plan is None:
                ...
                continue
            if self.transport_plan[_idx_s, :][:, _idx_t].sum() == 0:
                mmd += 0.0
                continue
            xy_transport = (
                self.transport_plan[_idx_s, :][:, _idx_t]
                / self.transport_plan[_idx_s, :][:, _idx_t].sum()
            )
            x_marginal = self.transport_plan[_idx_s, :].sum(axis=1, keepdim=True)
            xx_transport = x_marginal * x_marginal.T
            xx_transport /= xx_transport.sum()
            y_marginal = self.transport_plan[:, _idx_t].sum(axis=0, keepdim=True).T
            yy_transport = y_marginal * y_marginal.T
            yy_transport /= yy_transport.sum()
            mmd += (
                (XX * xx_transport).sum()
                + (YY * yy_transport).sum()
                - 2.0 * (XY * xy_transport).sum()
            )
        return mmd / len(torch.unique(label_index))

    def loss_cross_entropy(self, z, batch_index, obs_index, softmax_tmp: float = 1.0):
        """Loss for matching modality.
        obs_index: index of sample to be used to fetch the relevant part in self.transprot_plan
        batch_index: indicates if the sample z is from P or Q.
        label_index: indicates the label of z for the MMD to be calculated.
        """
        z0 = z[batch_index[:, 0] == 0, :]
        z1 = z[batch_index[:, 0] == 1, :]
        obs_index0 = obs_index[batch_index[:, 0] == 0, 0]
        obs_index1 = obs_index[batch_index[:, 0] == 1, 0]
        simscore = F.cosine_similarity(z1[None, :, :], z0[:, None, :], dim=-1)

        ce_loss = CrossEntropyLoss()
        target = self.transport_plan.to(z.device)[obs_index0, :][:, obs_index1]

        norm = target.sum(axis=1)[:, None]
        norm[norm == 0] = 1e-6
        return ce_loss(simscore, target / norm)
    ...
    def training_step(self, batch, batch_idx):
        ...
        kappa_match = (
            1 - self.kl_weight
            if self.scale_match_loss == "auto"
            else self.scale_match_loss
        )

        batch_tensor = batch[REGISTRY_KEYS.BATCH_KEY]
        label_tensor = batch[REGISTRY_KEYS.LABELS_KEY]
        index_tensor = batch[REGISTRY_KEYS.INDICES_KEY]
        ...
        assert self.adversarial_classifier is not None
        if kappa_adv > 0 and self.adversarial_classifier is not False:
            fool_loss = self.loss_adversarial_classifier(
                z, batch_tensor, label_tensor, False
            )
            loss += fool_loss * kappa_adv

        # if kappa_match > 0 and self.match is not False:
        #     if self.match_metric == "mmd":
        #         mmd_loss = self.loss_MMD(z, index_tensor, batch_tensor, label_tensor)
        #         loss += mmd_loss * kappa_match
        #     elif self.match_metric == "cross_entropy":
        #         ce_loss = self.loss_cross_entropy(z, batch_tensor, index_tensor)
        #         loss += ce_loss * kappa_match
        ...
        if opt2 is not None:
            loss = self.loss_adversarial_classifier(
                z.detach(), batch_tensor, label_tensor, True
            )
            loss *= kappa_adv
            opt2.zero_grad()
            self.manual_backward(loss)
    ...
    def configure_optimizers(self):
        ...
        assert self.adversarial_classifier is not False
        if self.adversarial_classifier is not False:
            params2 = filter(
                lambda p: p.requires_grad, self.adversarial_classifier.parameters()
            )
