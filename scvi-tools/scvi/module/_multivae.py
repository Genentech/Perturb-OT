...
from scvi.nn import Decoder, DecoderSCVI, Encoder, FCLayers, one_hot
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.output = torch.nn.Sequential(
            torch.nn.Linear(n_hidden, 1), torch.nn.Softplus()
        )
...
class MULTIVAE(BaseModuleClass):
    ...
    def __init__(
        ...
        accessibility_likelihood: Literal["normal", "binomial"] = "binomial",
        ...
        if self.gene_likelihood == "normal":
            self.z_decoder_expression = Decoder(
                n_input_decoder,
                n_input_genes,
                n_cat_list=cat_list,
        ...
        # accessibility
        self.accessibility_likelihood = accessibility_likelihood
        if self.accessibility_likelihood == "normal":
            self.pv = torch.nn.Parameter(torch.randn(n_input_regions))
        ...
        if self.accessibility_likelihood == "normal":
            self.z_decoder_accessibility = Decoder(
    ...
    def inference(
        ...
        if self.gene_likelihood != "normal":
            ...
            mask_expr = ~torch.isnan(x_rna).all(dim=1)
            assert not torch.isnan(x_rna[mask_expr, :]).any()
        if self.accessibility_likelihood != "normal":
            mask_acc = x_chr.sum(dim=1) > 0
        else:
            assert (
                x.shape[1] == self.n_input_genes + self.n_input_regions
            )  # @TODO remove
            mask_acc = ~(
                torch.isnan(x_chr).all(dim=1)
            )  # row will all NaNs is assigned of False
            assert not torch.isnan(x_chr[mask_acc, :]).any()
        ...
        # Z Encoders
        # assert not torch.isnan(x_chr).any(), x_chr
        qzm_acc, qzv_acc, z_acc = self.z_encoder_accessibility(
            torch.nan_to_num(encoder_input_accessibility),
            batch_index,
            *categorical_input,
        )
        qzm_expr, qzv_expr, z_expr = self.z_encoder_expression(
            torch.nan_to_num(encoder_input_expression), batch_index, *categorical_input
        )
    ...
    def _get_generative_input(self, tensors, inference_outputs, transform_batch=None):
        ...
        assert self.accessibility_likelihood == "normal"  # @TODO remove
        if self.accessibility_likelihood == "normal":
            input_dict["libsize_acc"] = inference_outputs["libsize_acc"]
        return input_dict
    ...
    def generative(
        ...
        # Accessibility Decoder
        if self.accessibility_likelihood == "normal":
            p, _ = self.z_decoder_accessibility(
                decoder_input, batch_index, *categorical_input
            )
            pv = (
                self.pv
            )  # torch.tensor(0.01, device=z.device)  # torch.exp(self.region_factors)
            p = p * libsize_acc
        else:
            p = self.z_decoder_accessibility(
                decoder_input, batch_index, *categorical_input
            )

        # Expression Decoder
        assert not self.use_size_factor_key  # @TODO remove
        if not self.use_size_factor_key:
            size_factor = libsize_expr
        if self.gene_likelihood == "normal":
            px_rate, _ = self.z_decoder_expression(
                decoder_input,
                batch_index,
                *categorical_input,
            )
            px_rate = px_rate * size_factor
            assert not torch.isnan(px_rate).any()
        else:
        ...
        assert self.gene_dispersion == "gene"  # @TODO remove
        ...
        if self.gene_likelihood != "normal":
            output["px_scale"] = px_scale
            # output["px_r"] = torch.exp(self.px_r)
            output["px_dropout"] = px_dropout
        if self.accessibility_likelihood == "normal":
            output["pv"] = torch.exp(pv)
        return output
    ...
    def loss(
        ...
        if self.accessibility_likelihood != "normal":
            ...
            mask_acc = ~torch.isnan(x_chr).all(dim=1)
        ...
        if self.accessibility_likelihood == "normal":
            pv = generative_outputs["pv"]
            rl_accessibility = self.get_reconstruction_loss_normal(
                torch.nan_to_num(x_chr), p, pv
            )
        else:
        ...
        if self.gene_likelihood == "normal":
            rl_expression = self.get_reconstruction_loss_normal(
                torch.nan_to_num(x_rna), px_rate, px_r
            )
        else:
        ...
        # if self.gene_likelihood == "normal":
        #     recon_loss_expression = torch.zeros(x_rna.shape[0], device=x_rna.device)
        #     recon_loss_expression[mask_expr] = rl_expression
        # else:
        recon_loss_expression = rl_expression * mask_expr
        # if self.accessibility_likelihood == "normal":
        #     recon_loss_accessibility = torch.zeros(x_chr.shape[0], device=x_chr.device)
        #     recon_loss_accessibility[mask_acc] = rl_accessibility
        # else:
    ...
    def get_reconstruction_loss_normal(self, x, mu, var, var_eps=1e-4):
        rl = (
            -Normal(
                loc=mu,
                scale=0.1,  # torch.sqrt(var),  # + torch.tensor(var_eps, device=var.device)
            )
            .log_prob(x)
            .sum(dim=-1)
        )
